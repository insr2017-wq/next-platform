...
function normalizeRow(
  ...
  return {
    ...
    initialUserBalance: Math.max(0, n(row.initialUserBalance, DEFAULTS.initialUserBalance)),
...
async function ensureInitialUserBalanceColumn(): Promise<void> {
  try {
    await prisma.$executeRawUnsafe(`
      ALTER TABLE "PlatformSettings"
      ADD COLUMN IF NOT EXISTS "initialUserBalance" DOUBLE PRECISION NOT NULL DEFAULT 0
    `);
  } catch (e) {
    console.error("[platform-settings] ensureInitialUserBalanceColumn:", e);
  }
}
...
> {
  try {
    await ensureInitialUserBalanceColumn();
      ...
      try {
        row = await prisma.platformSettings.create({
          data: {
            ...
            initialUserBalance: DEFAULTS.initialUserBalance,
