...
import { formatPixPhoneKeyForVizzionPay } from "@/lib/pix-phone-key";
import {
...
export function mapWithdrawalPixKeyTypeToVizzionPay(
  pixKeyType: string
...
export function formatPixKeyForTransfer(pixKeyType: string, pixKey: string): string {
  ...
  if (apiType === "phone") {
    return formatPixPhoneKeyForVizzionPay(pixKey);
  }
