...
async function resolveDepositFromWebhook(
  ...
  for (const ref of txnRefs) {
    ...
    const byTx = await prisma.deposit.findFirst({
      where: { gatewayProvider: { in: vizzionPayProviderValues() }, gatewayTransactionId: ref },
      select: { id: true },
    });
    if (byTx) {
      logVizzionPayPixEvent("webhook_pix_deposit_resolved", {
    ...
    const byExt = await prisma.deposit.findFirst({
      where: { gatewayProvider: { in: vizzionPayProviderValues() }, externalReference: ref },
      select: { id: true },
    });
    if (byExt) {
      logVizzionPayPixEvent("webhook_pix_deposit_resolved", {
  ...
  if (pixCopy) {
    const byPix = await prisma.deposit.findFirst({
      where: {
        gatewayProvider: { in: vizzionPayProviderValues() },
        status: "pending",
        pixCode: pixCopy,
      },
      select: { id: true },
...
export async function processVizzionPayPixWebhook(json: unknown): Promise<void> {
  ...
  try {
    await applyNormalizedWebhookEvent(
      {
        gatewayTransactionId: gatewayTransactionId ?? resolved.depositId,
        type: "deposit",
        status: "completed",
        amount: null,
        fee: null,
      ...
      { depositId: resolved.depositId, paidAt: canonical.paidAt }
    );
