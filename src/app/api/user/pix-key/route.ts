...
import { validateHolderName } from "@/lib/holder-name-validation";
import {
  extractLocalPhoneDigits,
  isValidLocalPhoneDigits,
  normalizePixPhoneKeyForStorage,
} from "@/lib/pix-phone-key";
...
function validatePhoneDigits(raw: string): boolean {
  return isValidLocalPhoneDigits(extractLocalPhoneDigits(raw));
}
...
export async function PATCH(request: Request) {
  ...
  if (pixKeyType === "telefone") {
    if (!validatePhoneDigits(pixKeyRaw)) {
      return NextResponse.json({ error: "Telefone inválido (formato)."} , { status: 400 });
    }
  }

  const pixKeyToStore =
    pixKeyType === "telefone" ? normalizePixPhoneKeyForStorage(pixKeyRaw) : digits;

  const holderNameRaw = typeof b.holderName === "string" ? b.holderName : "";
  const holderCpfRaw = typeof b.holderCpf === "string" ? b.holderCpf : "";
  const holderCpfDigits = normalizeDigits(holderCpfRaw);

  const holderNameResult = validateHolderName(holderNameRaw);
  if (!holderNameResult.ok) {
    return NextResponse.json({ error: holderNameResult.error }, { status: 400 });
  }
