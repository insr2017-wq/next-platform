...
export async function POST(request: Request) {
  ...
  // Normaliza chave Pix: CPF só dígitos; telefone com prefixo +55.
  const pixKeyDigits = pixKey.replace(/\D/g, "");
  const pixKeyStored =
    pixKeyType === "telefone" ? normalizePixPhoneKeyForStorage(pixKey) : pixKeyDigits;
  const holderCpfDigits = holderCpf.replace(/\D/g, "");

  if (pixKeyType === "telefone" && !isValidLocalPhoneDigits(extractLocalPhoneDigits(pixKey))) {
