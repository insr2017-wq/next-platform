...
import { mapGatewayWithdrawStatus } from "@/lib/gateways/status";
import { MISTICPAY_ID, VIZZION_PAY_ID } from "@/lib/gateways/types";
import { vizzionPayWithdrawWebhookUrl } from "@/lib/gateways/vizzion-pay";
...
export async function POST(
  ...
  const skipApi = gateway.id === VIZZION_PAY_ID && shouldSkipVizzionPayWithdrawTransfer();
  ...
  if (gateway.id === MISTICPAY_ID && !isMisticPayConfigured()) {
    return NextResponse.json(
      {
        error:
          "MisticPay não configurada. Defina MISTICPAY_CLIENT_ID e MISTICPAY_CLIENT_SECRET, ou selecione outro gateway no painel.",
      },
      { status: 503 }
    );
  }
  ...
  try {
    const outcome = await gateway.createWithdraw({
      amount: Number(withdrawal.netAmount),
      pixKey: withdrawal.pixKey,
      pixKeyType: withdrawal.pixKeyType,
      description: "Saque",
      webhookUrl:
        gateway.id === MISTICPAY_ID ? misticPayWebhookUrl() : vizzionPayWithdrawWebhookUrl(),
      holderName: withdrawal.holderName,
      holderDocument: withdrawal.holderCpf,
      requesterIp: parseUserIpFromWithdrawalExternalReference(withdrawal.externalReference),
      internalId: withdrawal.id,
    });

    const internal = mapGatewayWithdrawStatus(outcome.status);
