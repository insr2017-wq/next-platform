...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      initialUserBalance: s.initialUserBalance,
...
export async function PUT(request: Request) {
  ...
  const initialUserBalance = parseNonNegative(b.initialUserBalance);
  ...
  if (initialUserBalance === null) {
    return NextResponse.json(
      { error: "Saldo inicial de novos usuários inválido. Use um número maior ou igual a zero." },
      { status: 400 }
    );
  }
