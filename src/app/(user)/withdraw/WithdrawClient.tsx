...
import {
  HOLDER_NAME_INVALID_MESSAGE,
  isHolderNameInputAllowed,
  validateHolderName,
} from "@/lib/holder-name-validation";
import { extractLocalPhoneDigits } from "@/lib/pix-phone-key";
...
function MoneyField({
  ...
  return (
    ...
    >
      <span style={{ fontSize: 14, fontWeight: 900, color: "var(--text-muted)" }}>
        R$
      </span>
      <input
        type="text"
...
export function WithdrawClient({
  ...
  const initialCpfDigits =
    initialPixKey && initialType === "cpf" ? onlyDigits(initialPixKey) : "";
  const initialPhoneLocalDigits =
    initialPixKey && initialType === "telefone" ? extractLocalPhoneDigits(initialPixKey) : "";
  ...
  const [pixKeyCpf, setPixKeyCpf] = useState(
    initialCpfDigits ? maskCPF(initialCpfDigits) : ""
  );
  const [pixKeyPhone, setPixKeyPhone] = useState(
    initialPhoneLocalDigits ? maskPhone(initialPhoneLocalDigits) : ""
  );
  ...
  const initialHasSavedPixKey =
    ...
    Boolean(initialType === "telefone" ? initialPhoneLocalDigits : initialCpfDigits) &&
  ...
  function handleHolderNameChange(value: string) {
    if (!isHolderNameInputAllowed(value)) {
      setError(HOLDER_NAME_INVALID_MESSAGE);
      return;
    }
    setFullName(value);
    setError("");
  ...
  async function handleSavePixKey() {
    ...
    const holderNameResult = validateHolderName(fullName);
    if (!holderNameResult.ok) {
      setError(holderNameResult.error);
      return;
    }
    const holderName = holderNameResult.value;
